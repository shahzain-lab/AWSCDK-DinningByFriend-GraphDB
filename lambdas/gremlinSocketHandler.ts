...
let connection: driver.DriverRemoteConnection;
...
export async function gremlinQueryHandler(queryHandler: () => Promise<any>) {
    ...
    const createGraphTraversalSource = (connection: driver.DriverRemoteConnection) => {
        return gprocess.traversal().withRemote(connection);
    };

    if (connection == null) {
        connection = createRemoteConnection();
        g = createGraphTraversalSource(connection);
    }
